...
> 目标读者：Leader 需将此段落直接引用到 `code-submission-note` 或最终交付清单，用于说明 Platform + Runtime Server 的工程交付状态。

## 1. 最低运行前提

1. 启动 Runtime Server 服务（脚本会派生 `Research/` 等路径，参见 `Runtime-Server/README.md` 与 `Runtime-Server/LOCAL-INTEGRATION.md`）：
   ```powershell
   powershell -ExecutionPolicy Bypass -File Runtime-Server\run-local-api.ps1 -WorkspaceRoot D:\Code\DiffAudit
   ```
...
2. 同一工作区中，以 `D:\Code\DiffAudit\Platform` 为根先后启动 API 代理和前端：
   ...
   npm run dev:web
   ```
   API 代理 (`dev:api`) 必须先起，以便 `/api/v1/*` 在刷新时可即时路由到 Runtime Server。
...
1. 通过 curl 或前端的 `Get job template` 按钮查询活跃合同，例如 `black-box/recon/sd15-ddim`；模板地址说明见 `Runtime-Server/LOCAL-INTEGRATION.md`。
...
2. Runtime Server 当前在 `codex/portable-mainline` 分支，涉及 `README.md`、registry、config、runner README 等多份调整以及 `run-local-api.ps1` 脚本；如果需要将该枝杈同步到 `main`，需手动确认 `go test ./...`、runner Docker 构建等验证通过后再合并，否则 4C 包中只引用该状态说明。

附录：所有调度说明与镜像信息都散见于 `Runtime-Server/README.md`、`Runtime-Server/LOCAL-INTEGRATION.md` 以及 `Platform/docs/recording-prep.md`，Leader 可按需附上对应链接。
