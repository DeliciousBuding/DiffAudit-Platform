...
1. 启动顺序（照着做，确保请求路径都连通）：
   - 先在 `Runtime-Server` 目录运行 `run-local-api.ps1 -WorkspaceRoot D:\Code\DiffAudit` 并保持窗口常驻（监听 127.0.0.1:8765）。
   - 在 `Platform` 根下依次执行 `npm run dev:api`、`npm run dev:web`，先起 proxy，再起 Next.js，保证 `/api/v1/*` 实际指向 Runtime Server。
2. 登录态前提：
...
3. 最小 demo 闭环（演示必须有这三个节点）：
   - 有一个 job template 在 Runtime Server 里：必要时按 `Runtime-Server/LOCAL-INTEGRATION.md` 提供的 curl 把 job template 放入 `/api/v1/audit/jobs`。
...
5. 若须重录/重拍，先检查：
   - Runtime Server、`npm run dev:api`、`npm run dev:web` 三个窗口都在跑，并且日志没报 500。
