...
- `Runtime-Server/`：提供本地审计服务、合同注册库、Runner 资源与 HTTP contract 描述，是前端与 Research 之间的集成边界。  
...
- **Runtime-Server** `D:\Code\DiffAudit\Runtime-Server\README.md`：推荐用 `powershell -ExecutionPolicy Bypass -File .\run-local-api.ps1`（或 `go run ./cmd/local-api --host 127.0.0.1 --port 8765`），README 还有 Docker build/run、环境变量、HTTP contract 与 runner 配置的说明。  
...
- `Runtime-Server` 目前在 `codex/portable-mainline`，涉及配置、registry、runner、文档等的更新，该分支也没有合入 `main`。
