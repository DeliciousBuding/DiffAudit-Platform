...
1. **契约驱动的 job 流**  
   ...
   - 工程证据：`Services/Local-API/LOCAL-INTEGRATION.md` 的 job 模板、`Platform/docs/code-submission-note.md` 中的 API 路径与成功响应示例、`demo-preflight-checklist.md` 里同步的运行顺序与登录要求。  
   - 最后提交前边界：确保 `diffaudit_session` Cookie、Local API + API 代理都持续可用，且 demo 录屏中暴露的 job payload schema 与最终 docx 中 traceability 记录一致。
...
3. **工程预提交一致性**  
   ...
   - 工程证据：上述两个预提交文档直接可被 4C 材料引用，说明 Platform + Local-API 状态已满足工程预提交要求。  
