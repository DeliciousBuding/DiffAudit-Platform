...
export async function submitAuditJob(contractKey: string): Promise<JobSubmissionResult> {
  ...
  const jobRecord = (await createResponse.json()) as JobCreationResponse;
  const jobId = jobRecord?.job?.job_id ?? jobRecord?.job_id;
  const status = jobRecord?.job?.status ?? jobRecord?.status;
  if (!jobId) {
