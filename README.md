# DiffAudit Platform

`Platform` 是 DiffAudit 的唯一产品主线仓库。

公网地址：`https://diffaudit.vectorcontrol.tech/`

如果任务涉及 `gz2` 运行时、发布、服务重启或公网故障，先读：

- `RUNBOOK.md`
- `docs/public-runtime-runbook.md`
- `docs/public-runtime-handoff.md`

当前仓库承载：

- 单站前端：`apps/web`
- 当前 Go 网关：`apps/api-go`
- 共享契约与提示：`packages/shared`
- 部署、架构与协作文档：`docs`

首页、登录、工作台、审计流程、报告和设置都收口在同一个 Next.js 网站里。

## Product Surface

固定页面路径：

- `/`
- `/login`
- `/trial`
- `/workspace`
- `/workspace/audits`
- `/workspace/reports`
- `/workspace/settings`

## Design Source

- 主 Figma 文件：`DiffAudit`
- 协作与 MCP 规则见 `docs/figma-workflow.md`

## Local Start

前端：

```powershell
npm --prefix apps/web install
npm --prefix apps/web run dev
```

后端：

```powershell
npm run dev:api
```

公网默认链路：

- `apps/web` 只连接 `apps/api-go`
- `apps/api-go` 对展示态优先读取 `apps/api-go/data/public/*` snapshot
- 只有审计控制面动作才继续转发到 `Runtime`
- 当前 live 工作树是 `/home/ubuntu/projects/diffaudit/platform-web`

刷新公网 snapshot：

```powershell
npm run publish:public-snapshot
```

常用环境变量：

- `DIFFAUDIT_SHARED_USERNAME`
- `DIFFAUDIT_SHARED_PASSWORD`
- `DIFFAUDIT_PLATFORM_URL`
- `DIFFAUDIT_API_BASE_URL`
- `DIFFAUDIT_PUBLIC_DATA_DIR`
- `DIFFAUDIT_RUNTIME_BASE_URL`
- `DIFFAUDIT_DB_PATH`
- `GITHUB_CLIENT_ID`
- `GITHUB_CLIENT_SECRET`

说明：

- `DIFFAUDIT_SHARED_USERNAME` / `DIFFAUDIT_SHARED_PASSWORD` 现在用于首个共享账号引导。
- 当 SQLite 用户库为空时，登录接口会自动把这对共享账号写入用户表，便于公网首启或替换旧运行时时平滑过渡。
- 会话已经不再依赖固定的 `DIFFAUDIT_SESSION_TOKEN`。
- `DIFFAUDIT_API_BASE_URL` 在公网应指向本机 `apps/api-go`，即 `http://127.0.0.1:8780`，不再直接指向 `Runtime`。
- `DIFFAUDIT_RUNTIME_BASE_URL` 是 `apps/api-go` 探测与转发执行控制面的主变量；旧 `DIFFAUDIT_CONTROL_API_BASE_URL` 仅保留为兼容别名。

## Collaboration

默认协作流：

1. 从 `main` 拉短分支
2. 在当前工作树开发
3. 跑本地验证
4. push 分支
5. 提 PR
6. 1 个 review 后 squash merge 回 `main`

## Verification

前端标准验证：

```powershell
npm --prefix apps/web run test
npm --prefix apps/web run lint
npm --prefix apps/web run build
```

如涉及 Go 网关，再补：

```powershell
go -C apps/api-go test ./...
```

## 变更日志

### 近期变更

- **前端代码质量**：修复了多个 lint 警告，移除了未使用的变量和导入，提高了代码质量
- **API 文档**：为 Go 网关添加了详细的 Swagger 文档注释，覆盖所有主要 API 端点
- **错误处理**：完善了后端错误处理机制，确保所有错误都有标准化响应
- **代码审查**：进行了全面的代码审查，发现并修复了潜在问题
- **测试覆盖**：确保所有前端和后端测试都通过，提高了系统稳定性
- **功能验证**：修复了前端构建时的类型错误，确保系统正常工作

### 技术改进

- **前端**：使用 Next.js 16.2.2 和 React 19.2.4，采用 TypeScript 确保类型安全
- **后端**：使用 Go 1.26.1 构建 API 网关，实现了标准化的错误处理
- **API 文档**：集成 Swagger/OpenAPI 文档，提供详细的 API 端点说明
- **错误处理**：实现了统一的错误响应结构，包含错误码、提示信息等
- **测试**：完善了测试覆盖，确保所有功能都有对应的测试用例
